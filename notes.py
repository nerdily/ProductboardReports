...
def get_company_notes(token, company_id):
    ...
    all_company_notes_df = all_company_notes_df.rename(columns={"id": "note_id", "title": "note_title", "content":"note_content", "createdAt": "note_created_date", "owner":"note_pm", "company":"note_company", "features":"note_linked_features"})
    # Account for the possibility that a company has no notes. Just create an empty dataframe
    if all_company_notes_df.shape[0] == 0:
        all_company_notes_df = pd.DataFrame(columns=["note_id", "displayUrl", "note_created_date", "note_title", "note_content", "note_pm", "note_company", "note_linked_features"])  # Create an empty df if there's no notes for the company
